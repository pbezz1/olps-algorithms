...
def create_factor_df(assets_list, factor_name, isForwardFill):
    ...
    if(data is not None):
        ...
        if(isForwardFill):
            data = data.fillna(method='ffill', limit=3) #forward propagation of factors, with 3 as limit
...
def create_returns(data_path):
    ...
    for file in os.listdir(data_path+'close'):
        if(file.endswith('.csv')):
            ...
            if((os.stat(file_path).st_size != 0)):
                data = pd.read_csv(file_path,index_col=None,header='infer')
...
def get_only_PN(assets_list):
    ...
    new_list.sort(key=custom_key,reverse=False)
    return new_list
